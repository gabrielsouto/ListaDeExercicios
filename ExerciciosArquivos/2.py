...
if os.path.exists("usuarios.txt"):
    ...
    if len(usuarios_espacos) > 0:
        ...
        for indice_usuario_espaco in range(len(usuarios_espacos)):
            ...
            arquivo_relatorio.write(str(bytes_to_mb(int(espaco))).rjust(7)+" MB           ")
            arquivo_relatorio.write(str(porcentagem(int(espaco), espaco_total)).rjust(7)+ "%\n")
            
        arquivo_relatorio.write("\nEspaço total ocupado: " + str(bytes_to_mb(espaco_total)) + " MB\n")
        arquivo_relatorio.write("Espaço médio ocupado: " + str(bytes_to_mb(espaco_total/len(usuarios_espacos))) + " MB")
